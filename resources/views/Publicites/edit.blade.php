...
<head>
    ...
    <title>Modification des publicités</title>
    <style>
...
<body>
   <h1>Modifier la Publicité</h1>
    ...
    <form action="{{ route('publicites.update', $publicite) }}" method="POST" enctype="multipart/form-data">
        ...
        <input type="text" name="titre" value="{{ old('titre', $publicite->titre) }}" required><br><br>

        <label for="contenu">Contenu :</label>
        <textarea name="contenu" rows="4" required>{{ old('contenu', $publicite->contenu) }}</textarea><br><br>

        @if($publicite->image)
            <p>Image actuelle :</p>
            <img src="{{ asset('storage/'.$publicite->image) }}" alt="Image" width="200"><br><br>
        @endif
